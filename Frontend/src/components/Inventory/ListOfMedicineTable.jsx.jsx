...
export default function ReactVirtualizedTable(props) {
  ...
  // Fetch medicine data when component mounts or medicineArray changes
  useEffect(() => {
    const token = localStorage.getItem("token");
    console.log("This is the token", token);
    if (medicineArray.length > 0) {
      // Add medicine_info to each item in medicineArray
      ...
      axios
        ...
        .then((res) => {
          // Add medicine_info to each item in fetched data
  ...
  // Search medicines based on mdct_code and selected Unit Name
  useEffect(() => {
  ...
  // Handle changes in unit price input fields
  ...
  // Update filtered medicines with new unit prices
  useEffect(() => {
  ...
  // Handle deletion of a medicine - pop up form
  ...
  // Handle toggling of edit mode and saving of updated unit price
  ...
  return (
    ...
    <Paper style={{ height: "100%", width: "100%" }}>
      {/* Use TableVirtuoso for virtualized rendering of large data */}
      <TableVirtuoso
...
// Define List of Medicine table columns headers
const columns = [
...
const VirtuosoTableComponents = {
  ...
  TableHead: (props) => (
    <TableHead
      {...props}
      sx={{
        backgroundColor: "#A052E0", // Set the background color
      }}
    />
  ),
...
// Function to render fixed header content
...
function rowContent(
  ...
  // Highlight re-order level rows based on in-hand quantity
  const isBottle = 
  ...
  if (
    ...
    rowStyle.backgroundColor = "#F2FEA4"; // Adjust color for low quantity
  }
